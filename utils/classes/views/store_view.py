...
class StoreView(discord.ui.View):
    def __init__(self) -> None:
        ...
        self.add_item(discord.ui.Button(label="Never Store", url=STORE_URL, emoji="🏪"))

    @discord.ui.button(label="QR Code", emoji="🖇️", style=discord.ButtonStyle.blurple, custom_id="qrcode_create")
    async def qrcode_link(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            await interaction.response.send_message(f"⌛ Generando il QR code...", ephemeral=True)

            if not validators.url(STORE_URL):
            ...
            embedqrcode = discord.Embed(
                title="QR Code STORE",
                description=f"Ecco a te il QR code dello store ✅",
